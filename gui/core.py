...
if platform.system()=="Linux":
    # TODO: determine if texlive is installed
    try:
        import tex
        HAVE_LATEXPDF = True
    except ImportError:
...
EXPORT_LATEX_TEMPLATE = string.Template("""
\\title{ChemHelper Molecule Export}
\\author{
ChemHelper v$version
}
\\date{\\today}
...
\\hfill Generated by ChemHelper v$version
...
class Chemhelper(object):
    ...
    def initSubMenuMainMain(self):
        ...
        def f():
            ...
            if self.fname is None:
                return
            print("Saved to %s"%os.path.basename(self.fname))
            self.saveAs(self.fname)
                
        self.wmmt_save.addAction("click",f)
        
        # Add toolbar export
        self.wmmt_saveas = peng3d.gui.Button("saveasbtn",self.wmm_toolbar,self.peng.window,self.peng,
                                pos=lambda sw,sh,bw,bh: (self.wmmt_save.pos[0]+self.wmmt_save.size[0]+6,6),
                                size=[-1,32],
                                min_size=[96,32],
                                label=tl("chemhelper:main.main.saveas.label"),
                                borderstyle="oldshadow",
                                )
        self.wmm_toolbar.addWidget(self.wmmt_saveas)
        def f():
            fname = pyfileselect.saveDialog(FILE_TYPES)
            if fname is not None:
                self.fname = fname
                print("Saved to %s"%os.path.basename(fname))
                self.saveAs(fname)
        self.wmmt_saveas.addAction("click",f)
        
        # Add toolbar convert
        self.wmmt_convert = peng3d.gui.Button("convertbtn",self.wmm_toolbar,self.peng.window,self.peng,
                                pos=lambda sw,sh,bw,bh: (self.wmmt_saveas.pos[0]+self.wmmt_saveas.size[0]+6,6),
                                size=[-1,32],
                                min_size=[96,32],
        ...
        self.wmmt_export = peng3d.gui.Button("exportbtn",self.wmm_toolbar,self.peng.window,self.peng,
                                pos=lambda sw,sh,bw,bh: (self.wmmt_convert.pos[0]+self.wmmt_convert.size[0]+6,6),
                                size=[-1,32],
                                min_size=[96,32],
        ...
        def f():
            fname = pyfileselect.saveDialog(FILE_TYPES_EXPORT)
            if fname is not None:
                print("Exporting to %s"%os.path.basename(fname))
                self.exportTo(fname)
        self.wmmt_export.addAction("click",f)
        
        if HAVE_LATEXPDF:
            # Add toolbar print
            self.wmmt_print = peng3d.gui.Button("printbtn",self.wmm_toolbar,self.peng.window,self.peng,
                                    pos=lambda sw,sh,bw,bh: (self.wmmt_export.pos[0]+self.wmmt_export.size[0]+6,6),
                                    size=[-1,32],
                                    min_size=[96,32],
                                    label=tl("chemhelper:main.main.print.label"),
                                    borderstyle="oldshadow",
                                    )
            self.wmm_toolbar.addWidget(self.wmmt_print)
            def f():
                print("Printing")
                self.doPrint()
            self.wmmt_print.addAction("click",f)
    ...
    def exportTo(self,fname):
        if fname is None or fname=="":
            ...
            # JPEG
            mimetype = "image/jpeg"
        elif fname.endswith("bmp") or fname.endswith("bmp"):
            # Bitmap
            mimetype = "image/bmp"
        elif fname.endswith("latex") or fname.endswith("tex"):
            # LaTeX
        ...
        if mimetype=="application/x-latex":
            self.export_latex(fname)
        elif mimetype in ["image/png","image/jpeg","image/bmp"]:
            self.export_image(fname)
        elif mimetype=="application/pdf":
            self.export_pdf(fname)
    
    def export_latex(self,fname,save=True):
        pname = os.path.join(os.path.dirname(fname),"molecule.png")
        self.export_image(pname)
        
        # Generate IUPAC name
        # Currently only works in Lewis view
        if len(self.wmm_slewis.formula.atoms)==0:
            iupac = chemhelper.notations.iupac.IUPACNotation()
        else:
            iupac = self.wmm_slewis.formula.asIUPACName()
    ...
    def export_pdf(self,fname,save=True):
        # Temporary directory for the image
        dirname = tempfile.mkdtemp(prefix="chemhelper-pdfgen-")
        
        try:
            # Generate a fake file name and export the LaTeX file
            lname = os.path.join(dirname,"out.latex")
            latex = self.export_latex(lname,save=False)
            
            # Create the PDF using pdflatex
            ...
            print("Error while exporting as PDF")
            self.wmd_error.label_main=tl("chemhelper:main.error.label.export").format(exc=str(e),error=(e.args[0] if len(e.args)>=1 else "Unknown"))
            self.wmd_error.activate()
    ...
    def doPrint(self):
        pdf = self.export_pdf("/dev/null",save=False)
        
        # TODO: clean this up
        # TODO: add user feedback
        print("Printing...")
        # From https://stackoverflow.com/a/12725233
        # Modified based on https://stackoverflow.com/a/165662
        lpr = subprocess.Popen("/usr/bin/lpr", stdin=subprocess.PIPE)
        lpr.communicate(input=pdf)
        self.lpr = lpr # To prevent garbage collection
        print("Done!")
